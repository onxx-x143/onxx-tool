...
# ==============================================
# GATE-UP TOOLS INSTALLER v2.1
# Colorful, Interactive, Error‑proof
# ==============================================
...
EOF
    ...
    echo -e "${magenta}${bold}          Interactive Installer v2.1${reset}\n"
}
...
install_tool() {
    ...
    eval "$cmd" 2> error.log
    if [ $? -eq 0 ]; then
        echo -e "${green}✓ ${name} installed successfully!${reset}"
    else
        echo -e "${red}✗ ${name} failed! Check error.log${reset}"
        echo "Failed command for ${name}: ${cmd}" >> errors.txt
    fi
    echo ""
}
...
main_menu() {
    ...
    pkg install -y git curl wget python python-pip ruby php nodejs-lts golang figlet toilet
    ...
    echo -e "Logs saved in ${yellow}errors.txt${reset} (if any)."

    # Make script globally accessible
    cp "$0" "$PREFIX/bin/gateup" 2>/dev/null
    chmod +x "$PREFIX/bin/gateup" 2>/dev/null

    # Open GATE-UP URL in Chrome
    if command -v termux-open &> /dev/null; then
        termux-open "$GATEUP_URL" &
    elif command -v xdg-open &> /dev/null; then
        xdg-open "$GATEUP_URL" &
    else
        echo -e "${yellow}Could not open browser. Please visit: ${GATEUP_URL}${reset}"
    fi
